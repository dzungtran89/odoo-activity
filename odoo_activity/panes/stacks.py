...
from odoo_activity.probes import Worker, Thread
...
class StacksPane(Vertical):
    ...
    def show(self, workers: list[Worker], workdir: Path) -> bool:
        ...
        by_busy_count = sorted(
            workers, key=lambda w: sum(not t["idle"] for t in w["threads"]), reverse=True
        )
    ...
    def _add_thread(self, parent: TreeNode, thread: Thread, workdir: Path) -> None:
        ...
        where = (
            f" — {innermost['func']} ({_short(innermost['file'], workdir)}:{innermost['line']})"
            if innermost
            else ""
        )
