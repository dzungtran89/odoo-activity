...
def shell_command(inst: Instance, host: Host = LOCAL) -> str | None:
    """The instance's `odoo-bin shell --no-http` command, or None if it
    isn't running. Built from the master process's own argv (`procs_of`),
    inserting `shell --no-http` before its flags -- reuses the launched
    config/venv/interpreter as-is, and `--no-http` skips binding the port
    the running instance already holds (EADDRINUSE otherwise)."""
