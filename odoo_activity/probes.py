...
def odoosh_dumpstacks() -> str:
    """Trigger odoo.sh's own worker stack dump (written to the instance's
    odoo.log, same as `L`'s local SIGQUIT for the other managers)."""
    try:
        out = subprocess.run(["odoosh-dumpstacks"], capture_output=True, text=True)
    except FileNotFoundError:
        return "(odoosh-dumpstacks not found on PATH)"
