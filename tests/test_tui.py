...
def test_instance_action_waits_for_confirmation(monkeypatch):
    # regression guard: s/p/r and the buttons must not act until the user
    ...
    async def go():
        async with tui.OdooActivity().run_test(size=(100, 30)) as pilot:
            ...
            await pilot.press("s")  # action_start -> opens ConfirmScreen, no action yet
            await pilot.pause()
            assert calls == []
            ...
            await pilot.app.workers.wait_for_complete()
            await pilot.pause()
            assert calls == [("a.service", "start", "systemd")]
